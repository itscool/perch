...
## Cross-brand identification follow-up — 2026-09-07

Operational monitor profiles are keyed by exact EDID manufacturer/product IDs or explicitly reported model names, with input codes and evidence stored together. There are now 93 records, including LC-Power's unusual input mapping. These are community-documented mappings, not 93 Perch hardware certifications. The LG firmware catalog adds 162 identities, eight linked to evidenced input profiles; the others are identity-only.

- Dell: official DDPM macOS documentation requires DDC/CI and recommends a USB upstream connection for full functionality. This does not publish a universal alternate identity opcode. Existing Dell EDID records drive actual input choices. Some upstream records contain input-dependent high-byte values; these have deliberately not been imported without stronger write-code evidence. Source: https://www.dell.com/support/kbdoc/en-us/000201067/dell-display-and-peripheral-manager-for-macos
- MSI: `msigd` has a real firmware/model lookup (version plus special ID) and proprietary USB control. Its identity table is preserved in `vendor-protocol-research.json`, explicitly marked unsupported by Perch's DDC transport. Its README warns unsupported accesses can harm monitor operation; Perch does not probe that protocol. Source: https://github.com/couriersud/msigd
- NEC: the manufacturer's MIT SDK has a model-name query C217/reply C317 and firmware queries over LAN/RS232. These are not VCP opcodes. Recorded for a future separate transport; no network discovery or commands have been added. Source: https://github.com/NECDisplaySolutions/necpdsdk
- Eizo: some models implement standardized MCCS over USB HID. That requires a USB transport implementation and association with the correct display, not assuming every USB-C monitor exposes it. Source: https://www.ddcutil.com/usb/
