...
extension AppDelegate {
    ...
    func privacyOnlyReset(global: Bool) {
        let page = NSView(frame:NSRect(x:0,y:0,width:572,height:230))
        ...
        status.frame = NSRect(x:0,y:65,width:572,height:150); status.textColor = .secondaryLabelColor
        var active = true
        ...
        confirm.frame = NSRect(x:0,y:10,width:572,height:32)
        page.addSubview(status); page.addSubview(confirm)
    ...
    func systemResetPage() {
        let page = NSView(frame:NSRect(x:0,y:0,width:572,height:260))
        ...
        sleep.frame = NSRect(x:0,y:210,width:572,height:28)
        ...
        SettingsWindow.shared.show(.init(title:"Reset system sleep and audio",detail:"Select the exact changes to apply. Nothing is selected initially. macOS may ask for administrator authorization for sleep changes.",view:page))
    }
}
