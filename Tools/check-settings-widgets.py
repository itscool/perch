...
for width: CGFloat in [280, 640] {
    ...
    let editor = NSHostingView(rootView: SettingsShortcutEditor(title: "Fixture", enabled: .constant(true), key: .constant("F1"), choices: [("F1", "F1")], modifiers: .constant(.standard)).fixedSize(horizontal: false, vertical: true))
...
''')
    subprocess.run(['xcrun', 'swiftc', '-warnings-as-errors', *[str(perch_source(f)) for f in ['StatusColors.swift','SettingsFeedback.swift','Shortcut.swift','SettingsShortcutEditor.swift']], str(root/'main.swift'), '-o', str(root/'check')], check=True)
