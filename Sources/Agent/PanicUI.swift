...
extension AppDelegate {
    ...
    func makeAgentSettingsPage(mode: AgentSettingsPage.Mode = .agents, save: @escaping (SafetyConfiguration) throws -> Void = { try $0.save() }) -> AgentSettingsPage {
        AgentSettingsPage(mode: mode, save: save, conflicts: { shortcut in
            !ShortcutRegistry.perch.conflicts(with: shortcut, excluding: ShortcutRegistry.Source.emergency).isEmpty
    ...
    func runShortcutTest(readStatus: @escaping () -> SafetyStatus?, send: @escaping (String) throws -> Void, keepAlive: @escaping () throws -> Void, now: @escaping () -> Date = Date.init) {
        ...
        let heartbeat = MainTimer.every(1) { try? keepAlive() }
    ...
    @objc func safetyReport() {
        ...
        let log = SettingsLogView(frame: NSRect(x: 0, y: 0, width: 572, height: 490), monospaced: true, bordered: false, accessibilityLabel: "Agent safety report")
        log.replace("Preparing a fresh preview… No processes will be terminated.")
        var finished = false
        SettingsWindow.shared.show(.init(title: "Preview panic targets", detail: "A read-only preview of processes panic would attempt to terminate. Choose a settings category when you are finished.", view: log, poll: .init(every: 0.25, whileBusy: true) {
            guard !finished else { return }
            let modified = (try? FileManager.default.attributesOfItem(atPath: SafetyFiles.report.path)[.modificationDate]) as? Date
            if let modified, modified >= started, let report = try? String(contentsOf: SafetyFiles.report, encoding: .utf8) { log.replace(report); finished = true }
            else if Date().timeIntervalSince(started) > 5 { log.replace("The background helper did not return a preview. Check protection status in Settings."); finished = true }
        }))
    }
}
