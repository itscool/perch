...
@main struct KVMChecks {
    static func main() throws {
        ...
        let d = KVMInputLease.duration
        nonce = inputLease.challenge(now: 1000 + d - 0.1)
        try check(!inputLease.accept(grant, challenge: nonce, now: 1000.1 + d + 0.1), "late renewal cannot resurrect expired control")
        inputLease.release(); nonce = inputLease.challenge(now: 1002)
        try check(!inputLease.accept(grant, challenge: nonce, now: 1002 + d + 0.1), "queued grant cannot extend from arrival time")
        ...
        inputLease.acceptLocally(grant, now: 1005)
        try check(inputLease.alive(now: 1005.5), "owner installs its participant lease immediately")
        inputLease.release()
        ...
        var modifierHeld = KVMInputHeld()
        _ = modifierHeld.apply(.init(kind: .modifiers, code: 57, flags: 0x0001_0000), source: alice.id)
        let modifierReleases = modifierHeld.releaseAll()
        try check(modifierReleases.contains { $0.kind == .modifiers && $0.code == 57 }, "disconnect cleanup releases Caps Lock modifier state too")
