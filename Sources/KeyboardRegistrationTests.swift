...
func runKeyboardRegistrationUITests() throws {
    ...
    let root = host.pages.last!.view.subviews.compactMap { $0 as? NSButton }.first { $0.title.contains("Keyboard settings") }
    try check(root?.title.hasPrefix("⚠") == true, "Root Settings did not mark unknown keyboard")
