...
struct NavigationProbeKeyboard {
    ...
    static func connected() -> [NavigationProbeKeyboard] {
        let result: [NavigationProbeKeyboard] = HIDDevices.withAll { services in
            services.compactMap { service in
                guard let properties = IOServices.properties(service) else { return nil }
                ...
                guard let identity = NavigationKeyboardMetadata.identity(properties, ancestorBuiltIn: builtIn?.boolValue),
                      let id = IOServices.registryID(service) else { return nil }
                return .init(id: id, name: identity.name, transport: identity.transport, identity: identity)
            }
        } ?? []
...
final class NavigationProbeHID: NavigationProbeSource {
    ...
    func start(value: @escaping (UInt64, UInt32, UInt32, Int) -> Void, failed: @escaping (String) -> Void) throws {
        ...
        guard AccessCheck.hidListening else { throw AppError(message: "Enable Input Monitoring for Perch, then start the test again.") }
