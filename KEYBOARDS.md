...
Perch soft-links MachineSettings' read-only `createKeyForKeyboard` formatter to obtain exactly the same preference identity as Keyboard Settings, including older Apple `alt_handler_id` identities. The setting read/write APIs are public IOKit and CFPreferences APIs. The formatter is private and may change in a future macOS release; its absence disables changes with an explanation. Verified on the target macOS 26 system: built-in `0-0-0`, MX Keys `1133-45915-0`. No modifier keystrokes are intercepted by Perch 1.1's input helper.
