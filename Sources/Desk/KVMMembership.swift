...
struct KVMDeskArchive: Codable {
    ...
    func write(_ url: URL) throws {
        ...
        guard SecureFile.isDirect(directory) else { throw KVMError("Desk storage must not be a symbolic link.") }
        // Reopen after app/session restarts using macOS after-login protection,
        // retaining the owner-only directory/file modes.
        try SecureFile.writeAtomically(bytes, to: url, protection: true)
    }
}
