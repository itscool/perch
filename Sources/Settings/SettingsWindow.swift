...
final class SettingsWindow: NSObject, NSWindowDelegate {
    ...
    /// it, and it never fires under a covering page or while a dialog is up.
    struct Poll {
        var interval: TimeInterval
        var whileBusy = false
        var body: () -> Void
        init(every interval: TimeInterval, whileBusy: Bool = false, _ body: @escaping () -> Void) {
            self.interval = interval; self.whileBusy = whileBusy; self.body = body
        }
    }
    private var polls: [ObjectIdentifier: Timer] = [:]
    func pollTimer(for view: NSView) -> Timer? { polls[ObjectIdentifier(view)] }
    private func startPoll(_ page: Page) {
        stopPoll(page.view)
        guard let poll = page.poll else { return }
        let view = page.view
        polls[ObjectIdentifier(view)] = MainTimer.every(poll.interval, tolerance: poll.interval / 5) { [weak self] in
            guard let self, self.pages.last?.view === view, poll.whileBusy || !self.interactionBusy else { return }
            poll.body()
        }
    }
    private func stopPoll(_ view: NSView) { polls.removeValue(forKey: ObjectIdentifier(view))?.invalidate() }
    /// A page leaves the stack: its poll stops, then it is told it left.
    /// Re-showing the same view keeps the page's own state.
    private func retire(_ page: Page, leaving: Bool = true) { stopPoll(page.view); if leaving { page.leave?() } }
    ...
    private func openDestination(_ destination: SettingsDestination, preservingReturn: Bool = false) {
        ...
        if preservingReturn, let parent = pages.last {
            ...
            while pages.count > 1 { retire(pages.removeLast()) }
        } else {
            pages.reversed().forEach { retire($0) }; pages.removeAll()
        }
    ...
    func show(_ proposed: Page) {
        ...
        if page.title == "Perch settings" {
            pages.reversed().forEach { retire($0) }
            pages.removeAll()
        }
        if pages.last?.title == page.title { let previous = pages.removeLast(); retire(previous, leaving: previous.view !== page.view) }
        pages.append(page); startPoll(page); display(page)
    }
    @objc func goBack() {
