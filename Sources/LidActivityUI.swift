    ...
    func refresh() {
        ...
        read { [weak self] result in
            ...
            case .success(let entries):
                ...
                self.status.stringValue = helper?.activityError ?? (helper?.fresh == true ? "\(self.entries.count) entries · \(helper!.armed ? "Lid protection active" : "Lid session off")" : "\(self.entries.count) saved entries · The lid helper is not currently confirmed. History may have gaps.")
