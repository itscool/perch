...
func runKeyboardRegistrationTests() throws {
    ...
    for (i, code) in functionCodes.enumerated() {
        try check(KVMShortcut(key: "F\(i + 1)").local?.key == code, "Desk offered function key does not map to its Mac virtual key")
    }
    try check(KVMShortcut(key: "F21").local == nil && KVMShortcut(key: "Esc").local == nil, "Unsupported Desk key gained a guessed Mac mapping")
