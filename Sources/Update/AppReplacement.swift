...
final class AppReplacementMonitor {
    ...
    init() {
        ...
        reader = AppReplacementReader(path: Bundle.main.bundleURL, running: running, identifier: info["CFBundleIdentifier"] as? String) { path in
            guard let requirement = CodeIdentity.designatedRequirement else { throw AppError(message: "No signing identity") }
            _ = try CodeIdentity.verifiedHash(of: path, requirement: requirement)
        }
    }
    func start() {
