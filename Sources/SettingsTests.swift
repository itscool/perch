...
func runSettingsTests() throws {
    ...
    app.configurePanic()
    try check(host.pages.count == 2 && host.window.windowNumber == identity, "Agent safety changed windows")
    ...
    host.goBack()
    try check(host.pages.count == 2 && host.pages.last?.title == "Safety settings" && EventCollectorSetup.shared.timer == nil, "Collector Back/lifecycle failed")
    host.goBack()
    ...
    print("PASS: one-window Settings → Agent safety → Collector and Back; Input Done returns to root; Advanced navigation; timers stop on leaving")
}
