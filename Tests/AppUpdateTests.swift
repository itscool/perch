...
func runAppUpdateTests() throws {
    ...
    var fail = false
    do { _ = try CodeIdentity.verifiedHash(of: target, requirement: "identifier \"local.scott.perch\"") } catch { fail = true }
...
private func runDisposableUpdateWorkerTest(cancel: Bool = false) throws {
    ...
    let identity = try CodeIdentity.verifiedHash(of: target, requirement: CodeIdentity.designatedRequirement!)
    ...
    guard !worker.isRunning, worker.terminationStatus == 0,
          ...
          try CodeIdentity.verifiedHash(of: target, requirement: CodeIdentity.designatedRequirement!) == identity else {
