...
enum GuardianInstall {
    ...
    static var messagingInstalled: Bool {
        for (jobLabel, name) in [(label, HelperStatusIPC.guardian), ("local.scott.perch.input", HelperStatusIPC.input)] {
            ...
            guard let data = try? Data(contentsOf: path),
                  ...
                  LaunchdJob.user(jobLabel).isLoaded() else { return false }
        }
        return true
    }
    ...
    static func writeJob(executable: URL) throws {
        ...
        let result = LaunchdJob.user(label).replace(with: plist)
        guard result.succeeded else { throw AppError(message: "Watcher installation \(result). Try Repair watcher in Agent Kill Switch.") }
        ...
        let inputResult = LaunchdJob.user(inputLabel).replace(with: inputPlist)
        guard inputResult.succeeded else { throw AppError(message: "Input helper installation \(inputResult).") }
    }
    static func protectExecutable() throws {
        ...
        let command = "/bin/mkdir -p '/Library/Application Support/Perch' && /usr/bin/ditto " + quote(SafetyFiles.helperApp.path) + " " + quote(destination) + " && /usr/sbin/chown -R root:wheel " + quote(destination) + " && /bin/chmod -R go-w " + quote(destination)
