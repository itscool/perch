...
final class SystemMonitor {
    ...
    func read() -> [SystemReading] {
        ...
        var gpu = "Unavailable"
        var gpuCores: Int?
        IOServices.withMatching("IOAccelerator") { services in
            if let service = services.first {
                if let cores = IOServices.property(service, "gpu-core-count") as? NSNumber, cores.intValue > 0 { gpuCores = cores.intValue }
                if let stats = IOServices.property(service, "PerformanceStatistics") as? [String:Any], let percent = stats["Device Utilization %"] as? NSNumber, (0...100).contains(percent.doubleValue) {
