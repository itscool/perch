...
func runReleaseUITests() throws {
    ...
    try check(["Requested", "Normal lid sleep", "Applies when", "protection stopped", "Status unknown"].contains(where: { lid.contains($0) }), "Lid wording does not explain its state")
    ...
    try check(!app.awakeItem.isEnabled && !app.lidItem.isEnabled, "Unowned system override became editable Perch protection")
    app.observedLidDisabled = false; app.observedSleep = SleepStatus(perchActive: false, caffeinateProcesses: []); UserDefaults.standard.set(false, forKey: SleepPreferences.lidPreferenceKey); app.applyLidSleepPresentation()
