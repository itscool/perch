...
struct LidOverrideStore {
    ...
    func read<T: Decodable>(_ type: T.Type, path: String) throws -> T {
        let data: Data
        do { data = try SecureFile.read(path, .init(owner: owner, maximumBytes: 2048, requirePrivate: true)) }
        catch { throw AppError(message: "The lid recovery record is invalid. " + error.localizedDescription) }
    ...
    func reserve(token: String, disabled: Bool) throws {
        ...
        guard !FileManager.default.fileExists(atPath: recordPath), !disabled else {
            throw AppError(message: "A system sleep override already exists. Reset Settings → Perch sleep protection can remove Perch-owned protection; overrides owned outside Perch must be cleared at their source.")
        }
...
enum LidSleepOverride {
    ...
    private static func run(_ mode: String, token: String) throws {
        ...
        do { output = try Subprocess.run(binary.path, ["--lid-override-worker", mode, token, String(LidGuardClock.now + 3)], timeout: 2) }
        catch is Subprocess.Timeout { throw AppError(message: "macOS power control timed out. The recovery record is retained for retry.") }
        guard output.succeeded else { throw AppError(message: "macOS power control failed. Review Lid activity; recovery will retry if needed.") }
    }
    static func set(_ enabled: Bool) throws {
        if enabled {
            if LidMaintenance.holding { try LidMaintenance.require(LaunchdJob.system(LidMaintenance.name).print(timeout: LidMaintenance.launchdTimeout)) }
    ...
    private static func requireRecoveryJob() throws {
        ...
        switch LaunchdJob.system(LidGuardInstall.recoveryName).print(timeout: 0.5) {
        case .ok: break
        case .timedOut: throw AppError(message: "Could not confirm independent sleep recovery is loaded.")
        case .failed, .error: throw AppError(message: "Independent sleep recovery is not loaded. Repair lid protection before enabling it.")
        }
    }
