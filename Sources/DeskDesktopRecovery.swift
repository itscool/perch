...
enum DeskDesktopRecovery {
    ...
    static func displays() -> [UInt32] {
        var ids = [UInt32](repeating: 0, count: 32), count: UInt32 = 0
    ...
    static func save(_ records: [DeskDesktopRecord]) throws {
        try SafetyFiles.write(records, to: journal)
        let handle = try FileHandle(forWritingTo: journal); defer { try? handle.close() }; try handle.synchronize()
        let directory = open(SafetyFiles.base.path, O_RDONLY); if directory >= 0 { defer { close(directory) }; guard fsync(directory) == 0 else { throw KVMError("Desktop recovery could not be synchronized.") } }
    }
