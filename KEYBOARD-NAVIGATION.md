...
## Build 10: menu controls and runtime

The previously missing switches are present under External keyboards. Both default off. The existing input helper adds key-down/up events to its tap only while navigation is requested (or a mapped key is awaiting release). Ordinary typing is rejected by numeric keycode before any lookup; callbacks perform no file reads, app lookup, process scan, event posting or keyboard-setting writes. A bounded 64-entry table preserves paired releases/repeats across preference/app changes. Device/configuration notifications refresh metadata outside the callback; app activation updates exception state.

The implementation uses private CoreGraphics field 87 as an exact sender ID, matched to native Passive event-service metadata and previously registered profiles. Missing/unmatched sources pass unchanged and produce a status warning for navigation keys; built-in sources always pass unchanged. This is an opportunistic API path, not a guarantee that all macOS keyboard drivers expose it. The original developer discussion identifying field 87 is at https://stackoverflow.com/questions/219623/device-information-from-nsevent-cgevent (Noah Nuebling's comments). Apple defines IOHID sender IDs as event-service registry IDs in IOHIDEventData.h. Keychange's current source also acknowledges the private sender path, but uses last-active-device inference instead; Perch does not use that inference.

No raw HID subscription is added to the helper. The menu app sends its validated profiles through existing configuration transport. Perch does not grant the development host input permissions. The raw guided registration test remains user-initiated inside Perch. Native suppression and a virtual driver are not used, so a crashed helper does not leave persistent disabled-key mappings.

Safe tests cover unknown/built-in separation, app exceptions, paired repeats/releases, changes while held, malformed records, old helper-status compatibility, and no transformation of unrelated input. Physical source metadata, actual editing behavior and secure-input behavior have not yet been verified on build 10. A user-operated check of the installed switches is required; do not describe the feature as hardware-verified solely from mock tests. Earlier build-8/9 sections above describe historical milestones, not the current implementation.
