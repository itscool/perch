...
final class AgentGuardian {
    ...
    func tick() {
        ...
        // Keep awake belongs to a running Perch; a guardian kept to enforce a block releases it.
        do { try awake.set(config.keepAwake && !FileManager.default.fileExists(atPath: HelperLifecycle.closedMarker.path)) } catch { statusError = error.localizedDescription }
        events.tick()
    ...
    func resume() {
        do {
            let failed = try AgentJobRecovery.resume(state: &state, persist: { try SafetyFiles.writeRecoveryState($0) }, enable: { label in
                let result = LaunchdJob.user(label).enable().description
    ...
    func disableAgentJobs() {
        ...
        for dir in dirs {
            for url in (try? FileManager.default.contentsOfDirectory(at: dir, includingPropertiesForKeys: nil)) ?? [] where url.pathExtension == "plist" {
                ...
                let disabled = LaunchdJob.disabledListing(in: .user(getuid()))
