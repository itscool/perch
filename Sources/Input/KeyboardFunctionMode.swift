...
enum ExternalKeyboardModes {
    static func keyboards(standard: Bool?, only names: Set<String>? = nil) -> [KeyboardModeResult] {
        HIDDevices.withAll { keyboards(standard: standard, only: names, services: $0) }
            ?? [.init(name: "External keyboards", detail: "⚠ Could not enumerate connected keyboards.", verified: false)]
    }
    private static func keyboards(standard: Bool?, only names: Set<String>?, services: [io_service_t]) -> [KeyboardModeResult] {
        let property = IOServices.property
        var results: [KeyboardModeResult] = []
        ...
        for service in services {
            ...
            guard AccessCheck.hidListening else {
