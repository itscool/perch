...
struct SleepPresentation: Equatable {
    ...
    init(ordinary: SleepStatus?, actualLid: NSControl.StateValue, savedLid: Bool,
        ...
        lidHint = changing ? "Changing…" : actualLid == .mixed ? "Status unknown · review settings" : actualLid == .on ? remaining.map { "Requested · \($0)s remaining" } ?? "Requested · unverified" : savedLid ? (masterWanted ? "Saved · protection stopped" : "Applies when Keep awake is on") : "Normal lid sleep"
    }
}
