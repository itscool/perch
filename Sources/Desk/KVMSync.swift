...
struct KVMSyncGraph {
    ...
    /// Revisions a peer cannot have yet, given the heads it reported. Heads this
    /// graph does not know contribute nothing, so a diverged peer receives the
    /// whole branch it lacks; a peer on the same head receives nothing.
    func missing(from peerHeads: [String]) throws -> [KVMSignedRevision] {
        var known = Set<String>()
        for head in peerHeads where revisions[head] != nil { known.insert(head); known.formUnion(ancestors[head] ?? []) }
        return try history().filter { !known.contains($0.id) }
    }
...
struct KVMMessageFramer {
    ...
    mutating func append(_ chunk: Data) throws -> [Data] {
        ...
        while buffer.count - offset >= 4 {
            ...
            messages.append(Data(buffer[offset + 4..<offset + 4 + length]))
            offset += length + 4
        }
        // Trim once per read instead of copying the remainder for every frame.
        if offset > 0 { buffer = Data(buffer[offset...]) }
        return messages
    }
}
