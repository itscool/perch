...
final class KVMInputSession: ObservableObject {
    ...
    func capture(_ event: KVMInputEvent) -> Bool {
        ...
        if localInputSuppressedUntil != 0 { localInputSuppressedUntil = 0 }
    ...
    private func receive(_ message: KVMInputMessage, peer: UUID) {
        ...
        case .prepare(let value):
            ...
            endLocal(); localInputSuppressedUntil = 0; preparedGrant = value; preparedAt = now
    ...
    func tick() {
        ...
        if let grant = lease.grant, !lease.alive(now: clock()) || !ready() {
            endLocal(); localInputSuppressedUntil = clock() + KVMInputLease.duration
            localProblem = "Input is paused while Perch confirms the shared lease. Press Ctrl-Opt-Esc for local control."; send(.stop(grant.id), to: node.ownerID)
        }
        let now = clock()
