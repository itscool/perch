...
func runSleepPresentationTests() throws {
    ...
    try check(host.pages.last?.title == "Keep awake" && !host.modal && !host.interactionBusy, "Lid setup failure trapped the user in a modal result")
    ...
    host.goBack()
    try check(host.pages.last?.title == "Setup & status" && host.feedback == nil && !host.detail.stringValue.contains("Fixture: repair failed"), "Lid repair return failed to restore the checklist and its own explanation")
    app.keyboardAccessRecovery()
