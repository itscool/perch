            ...
            do {
                ...
                try pending.withUnsafeBytes { raw in
                    try BorrowedProcessEvent.withBytes(UnsafeRawBufferPointer(rebasing: raw[offset..<(offset + lineCount)])) { event in
                        ...
                        if Int64(event.subject.token.pid) == Int64(probePID ?? -1) {
                            if seenProbePaths.count > 64 { seenProbePaths.removeAll(keepingCapacity: true) } // PID reuse must not grow this diagnostic set forever.
                            seenProbePaths.insert(event.subject.path.retainedString())
                        }
