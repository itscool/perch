...
final class KeyboardModeMonitor: NSObject {
    ...
    private let attachments = HIDAttachmentObserver()
    ...
    func start() {
        ...
        attachments.changed = { [weak self] in self?.queue(navigationChanged: true) }
        if !attachments.start() { connectionError = "Keyboard connection monitoring is unavailable. Recheck keyboards after connecting one." }
    ...
    @objc private func activated() {
        if needsAccess && AccessCheck.hidListening { queue(reapplyExternal: true) }
        else { queue() }
    }
