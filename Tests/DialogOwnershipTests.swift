...
func runDialogOwnershipTests() throws {
    ...
    // Window-owned polling: one timer per shown page, stopped by Back, by a
    // replacing page and by closing; re-showing the same view never doubles it.
    var polled = 0
    let polling = NSView(frame: NSRect(x: 0, y: 0, width: 572, height: 100))
    host.show(.init(title: "Polling", detail: "", view: polling, poll: .init(every: 60) { polled += 1 }))
    let first = host.pollTimer(for: polling)
    try check(first != nil && polled == 0, "A shown page did not start its poll")
    host.show(.init(title: "Polling", detail: "", view: polling, poll: .init(every: 60) { polled += 1 }))
    try check(first?.isValid == false && host.pollTimer(for: polling) != nil && host.pages.count == 2, "Re-showing a page kept its old poll timer")
    host.pollTimer(for: polling)?.fire()
    try check(polled == 1, "The visible page's poll did not run")
    host.show(.init(title: "Cover", detail: "", view: NSView()))
    host.pollTimer(for: polling)?.fire()
    try check(polled == 1, "A covered page's poll ran")
    host.goBack()
    host.pollTimer(for: polling)?.fire()
    try check(polled == 2 && host.pages.last?.title == "Polling", "Returning to a page did not resume its poll")
    let covered = host.pollTimer(for: polling)
    host.goBack()
    try check(covered?.isValid == false && host.pollTimer(for: polling) == nil, "Leaving a page kept its poll timer")
    host.show(.init(title: "Polling", detail: "", view: polling, poll: .init(every: 60) { polled += 1 }))
    host.windowWillClose(Notification(name: NSWindow.willCloseNotification, object: host.window))
