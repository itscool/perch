...
final class PerchUpdater: NSObject, SPUUpdaterDelegate, SPUStandardUserDriverDelegate {
    ...
    private func identity(_ item: SUAppcastItem) throws -> UpdateIdentity {
        guard let payload = item.propertiesDictionary["perch:identity"] as? String,
              ...
              let bundle = Bundle.main.bundleIdentifier, let requirement = CodeIdentity.designatedRequirement else {
    ...
    func terminationReply(_ sender: NSApplication, willExit: @escaping () -> Void) -> NSApplication.TerminateReply? {
        ...
        guard active else {
            willExit()
            TerminationReply.send(true, after: 0.3, to: sender)
        ...
        LidGuardClient.shared.prepareForRestart(identity: proposed.codeHash) { result in
            ...
            defer {
                ...
                TerminationReply.send(allowed, to: sender)
            }
            do {
    ...
    func completeLaunch(_ completion: @escaping () -> Void) {
        ...
        guard record.matches(boot: CollectorIdentity.bootID, path: Bundle.main.bundleURL.standardizedFileURL.path,
                             ...
                             identity: CodeIdentity.current, now: LidGuardClock.now) else {
    ...
    @objc func releaseHistory() {
        ...
        let log = SettingsLogView(frame: NSRect(x: 0, y: 0, width: width, height: 100), bordered: false)
        log.text.drawsBackground = false; log.text.font = .systemFont(ofSize: 13); log.text.textContainerInset = NSSize(width: 8, height: 8)
        log.replace(text)
        let measured = (text as NSString).boundingRect(with: NSSize(width: width - 28, height: .greatestFiniteMagnitude),
                                                        options: [.usesLineFragmentOrigin, .usesFontLeading],
                                                        attributes: [.font: log.text.font ?? NSFont.systemFont(ofSize: 13)]).height
        log.frame.size.height = max(100, ceil(measured) + 24)
        SettingsWindow.shared.show(.init(title: "Release history", detail: "Every published Perch release, including the current version.",
                                         view: log, preferredBodyHeight: min(600, max(300, log.frame.height)), preferredBodyWidth: width))
    }
}
