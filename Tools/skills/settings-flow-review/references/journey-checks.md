...
- When Back fails but window X works, trace both the event delivery and completion paths; equivalent handlers do not prove equivalent native input. Replay preparation → success/timeout → Back → cleanup → parent using the actual shared control without injected response values. For app-owned pages in a reusable window, prefer completion callbacks over successive native modal loops, retaining exclusive action ownership and bounded operational cleanup. Do not remove native OS picker/authorization behavior that is already coordinated. Navigation that pops several pages must detect a refused Back and stop, preserving the validation message and draft instead of looping forever.
...
- Treat external-app return as several distinct paths: app activation, clicking the existing window, reopening Settings from an accessory/status menu, and closing/reopening. Check missing or delayed activation notifications; a fixture that injects the expected notification only proves that handler. Passive handoff bookkeeping must not strand all navigation, while real authorization or modal operations must retain their legitimate ownership.
...
- For actions that need later restoration, trace the durable recovery record as well as the button flow. Persist and verify recovery intent before the external change, and persist a release decision before presenting it as released. Exercise failed initial/final writes, ambiguous command results and restart reconstruction with injected operations. Never silently discard a recovery record because a command timed out or a later save failed.
...
- Audit help coverage and meaning separately from hover mechanics. Inventory action rows, conditional states and matching Settings controls. Explain effect, scope and confirmation consistently; add a current blocker/repair route without replacing the purpose with raw status or an empty string. Use shared copy for the same concept (including shortcut key names) across menu, dialog and accessibility help. Keep necessary explanations visible; tooltips supplement labels and inline help, and obvious navigation need not repeat its label. A fixed tooltip ownership bug is not proof of complete or coherent help.
...
- Trace reset scope through every current persistence store, including shared group files outside ordinary preferences. Name local versus shared effects before confirmation; a local reset should not unexpectedly erase other members’ setup. A catalog of defaults is not a custom-item workflow: trace add, disable, reenable, remove, save failure and reopening.
- A whole-version conflict decision needs every meaningful value it replaces, including device mappings, codes, control routes, geometry, shortcuts and shared input identities. Two different operational configurations must not render as identical choices. For synchronized text, distinguish pristine fields from unfinished drafts; refresh the former and preserve/reconcile the latter. Numeric protocol fields need a complete-edit boundary so intermediate digits do not become operating values.
- Validate shared resources such as global shortcuts against the current feature registry in both editors, including remote changes. Preserve a working registration until acquiring its replacement, and distinguish the saved proposal from the shortcut actually registered. Do not report registration success from persistence alone.
...
- Follow keyboard focus and text selection through an informational alert, ordinary child, same-view redraw and replacement of a page. Use stable semantic control identifiers for replacements; do not infer focus restoration from a correct page title. Label editable fields, popup purposes and repeated per-item actions with their scope, rather than leaving a screen reader to announce only the current value. A mouse-only drag setup needs an equivalent accessible action and visible keyboard instructions; role/label metadata alone does not make dragging operable.
