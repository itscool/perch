...
final class HelperStatusServer: NSObject, NSXPCListenerDelegate, HelperStatusProtocol {
    ...
    func listener(_ listener: NSXPCListener, shouldAcceptNewConnection connection: NSXPCConnection) -> Bool {
        guard connection.effectiveUserIdentifier == getuid(), let requirement = CodeIdentity.designatedRequirement else { return false }
...
final class HelperStatusClient<Status: Decodable> {
    ...
    func refresh() {
        ...
        if connection == nil {
            guard let requirement = CodeIdentity.designatedRequirement else { let changed = !completedRead; completedRead = true; lock.unlock(); if changed { notifyChange() }; return }
