...
final class PermissionSetup: NSObject {
    ...
    func show(fromSettings: Bool = false) {
        ...
        presentedHeight = nil
        SettingsWindow.shared.show(.init(title: "Scrolling & navigation access", detail: "Current input access and controls are checked automatically. If setup needs attention, the next step appears here.", view: content, poll: .init(every: 0.5, whileBusy: true) { [weak self] in self?.refresh() }))
        refresh()
    }
    func refresh() {
    ...
    @objc func openSettings() {
        SettingsWindow.shared.openSystemSettings(.accessibility)
    }
