...
enum LidGuardOwnership {
    ...
    static var token: String? {
        guard let data = try? SecureFile.read(path, .init(owner: 0, maximumBytes: 64)), data.count == 36,
...
final class LidGuardService: NSObject, NSXPCListenerDelegate {
    ...
    func listener(_ listener: NSXPCListener, shouldAcceptNewConnection connection: NSXPCConnection) -> Bool {
        guard connection.effectiveUserIdentifier == owner, let requirement = CodeIdentity.designatedRequirement else { return false }
