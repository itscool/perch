...
extension AppDelegate {
    ...
    @objc func keepAwakeSettings() {
        ...
        let lid = page.add("Including with the lid closed", detail: "Set up with the lid open. Authorizes a supervised helper; check Ready before closing the lid.", checkbox: true) { [weak self] in self?.toggleLid() }
        ...
        page.update = { [weak self, weak page] in
            ...
            lid.state = guarded || self.observedLidDisabled == true ? .on : self.observedLidDisabled == nil || LidGuardClient.shared.status?.error != nil ? .mixed : .off
