...
final class AppUpdate {
    ...
    func restartCurrentApp() {
        ...
        do {
            ...
            guard let requirement = CodeIdentity.designatedRequirement else { throw AppError(message: "This Perch copy has no verifiable signing identity.") }
            let hash = try CodeIdentity.verifiedHash(of: target, requirement: requirement)
    ...
    func restart() {
        ...
        do {
            guard let requirement = CodeIdentity.designatedRequirement,
                  try CodeIdentity.verifiedHash(of: candidate.target, requirement: requirement) == candidate.identity else {
    ...
    static func readRecord(_ path: String) throws -> AppUpdateRecord {
        ...
        let data: Data
        do { data = try SecureFile.read(url.path, .init(maximumBytes: 16_383)) }
        catch { throw AppError(message: "Invalid update record. " + error.localizedDescription) }
        let record = try JSONDecoder().decode(AppUpdateRecord.self, from: data)
    ...
    static func runWorker(_ path: String) throws {
        ...
        guard CodeIdentity.current == c.identity, sameLocation(Bundle.main.bundleURL, c.target),
              let requirement = CodeIdentity.designatedRequirement else { throw AppError(message: "The prepared update identity changed.") }
        guard try CodeIdentity.verifiedHash(of: c.target, requirement: requirement) == c.identity else { throw AppError(message: "The prepared app changed before worker startup.") }
        ...
        defer {
            if !finished, launched?.isRunning != true, let hash = try? CodeIdentity.verifiedHash(of: c.target, requirement: requirement), hash == c.identity {
        ...
        guard try CodeIdentity.verifiedHash(of: c.target, requirement: requirement) == c.identity else { throw AppError(message: "The app changed before restart.") }
    ...
    static func completeLaunch(completion: @escaping () -> Void) {
        ...
        do {
            ...
            guard sameLocation(Bundle.main.bundleURL, c.target), CodeIdentity.current == c.identity else { throw AppError(message: "Restart identity did not match the prepared update.") }
