...
func runShortcutDialogBackTests() throws {
    ...
    let defaults = Shortcut()
    try check(defaults.key == UInt32(kVK_Escape) && defaults.modifiers == [.control, .option, .command] && defaults.title == "⌃⌥⌘Esc", "New-machine shortcut or shared spelling changed")
    let app = AppDelegate()
