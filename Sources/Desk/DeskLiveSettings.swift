...
struct DeskLiveSheet: View {
        ...
        VStack(alignment: .leading, spacing: 14) {
            ForEach(node.group.presets.indices, id: \.self) { i in
                SettingsShortcutEditor(title: node.group.presets[i].name,
                    ...
                    choices: ShortcutKey.desk.map { ($0.name, $0.name) },
                    modifiers: Binding(get: { node.group.presets[i].shortcut.modifiers }, set: { value in changeShortcut(i) { $0.modifiers = value } }))
            }
    ...
    func changeShortcut(_ i: Int, _ edit: (inout KVMShortcut) -> Void) {
        perform {
            var group = node.group; edit(&group.presets[i].shortcut)
            if let local = group.presets[i].shortcut.local, let problem = ShortcutRegistry.perch.problem(with: local, excluding: ShortcutRegistry.Source.deskPresets) { throw KVMError(problem) }
            try node.edit(group)
        }
    }
    ...
    var body: some View {
        if let runtime = coordinator.runtime { DeskMemberRoot(runtime: runtime, node: runtime.node) }
            ...
            VStack(alignment: .leading, spacing: 18) {
                ...
                Text("Set up monitor presets first. Keyboard and mouse sharing is on by default on each computer and can be turned off from the Perch menu there. Perch will ask for Accessibility and Input Monitoring access the first time.").foregroundStyle(.secondary)
...
extension AppDelegate {
    @objc func deskSettings() {
        ...
        SettingsWindow.shared.show(.init(title: "Desk", detail: "Arrange screens and edit presets. Play switches monitor inputs. Keyboard and mouse sharing follows the active preset automatically.", view: view, preferredBodyWidth: 720, layout: { [weak view] size in
            // Desk has its own graph scroller, so it normally fills the body
            // exactly. In a window too short for its header and preset cards
            // the page keeps its minimum height and the host's scroller
            // appears, instead of the top of the page being cut off.
            guard let view else { return }
            let minimum = view.fittingSize.height
            view.setFrameSize(NSSize(width: size.width, height: max(size.height, minimum)))
        }))
    }
}
...
extension AppDelegate {
    func refreshDeskSharingMenu() {
        ...
        let access = runtime.inputAdapter.accessProblem
        if let access, runtime.inputAdapter.needsPermissionSetup {
            // Missing access is the one thing only the user can fix: the row
            // takes them there instead of silently turning sharing off.
            item.action = #selector(sharingAccessSetup)
            (item.view as? MenuRowView)?.opensAnotherInterface = { true }
            label(item, "Share on this Mac", hint: "Needs access", hintColor: StatusColors.warning)
            item.menuHelp = access
            return
        }
        item.action = #selector(toggleDeskSharing)
        (item.view as? MenuRowView)?.opensAnotherInterface = { false }
        let problem = access ?? runtime.input.problem
        let hint = problem != nil ? "Needs attention" : !runtime.input.enabled ? "Off" : runtime.inputAdapter.note != nil ? "On · keys local" : "On"
        label(item, "Share on this Mac", hint: hint, hintColor: problem == nil ? .secondaryLabelColor : StatusColors.warning)
        item.menuHelp = problem ?? runtime.inputAdapter.note ?? "Allow approved Desk computers to send keyboard and mouse input to this Mac. An active preset with a remote screen starts control automatically."
    }
    ...
    func refreshDeskMenu() {
        ...
        for item in deskPresetItems {
            ...
            let issue = runtime.switching.busy ? nil : runtime.switching.readiness(preset.id)
            // The hint is derived from the same state as the Desk card: never
            // "Confirmed" for a switch the monitor could not confirm.
            let hint: String
            if runtime.switching.busy, runtime.switching.request?.preset == preset.id { hint = "Switching…" }
            else if runtime.switching.activePreset == preset.id { hint = runtime.model.activeUnconfirmed ? "Active · unconfirmed" : "Active" }
            else { hint = "" }
            label(item, preset.name, hint: hint)
            (item.view as? MenuRowView)?.shortcutHint = preset.shortcut.label
            item.isEnabled = issue == nil && !runtime.switching.busy
            item.menuHelp = issue ?? DeskWording.live.presetActivationHelp
        }
    }
