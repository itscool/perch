...
final class AgentSettingsPage: NSObject {
    ...
    var modifiers: [(NSButton, Shortcut.Modifiers)] = []
    private let load: () -> SafetyConfiguration
    private let save: (SafetyConfiguration) throws -> Void
    private let conflicts: (Shortcut) -> Bool
    private let didSave: () -> Void
    private enum Change { case agent(String, Bool), shortcut(Shortcut), reset(Int) }
    ...
    init(mode: Mode = .agents, load: @escaping () -> SafetyConfiguration = SafetyConfiguration.load,
         ...
         conflicts: @escaping (Shortcut) -> Bool, didSave: @escaping () -> Void = {},
        ...
        for (index, entry) in Shortcut.Modifiers.editable.enumerated() {
            let box = NSButton(checkboxWithTitle: entry.name, target: self, action: #selector(shortcutChanged))
            ...
            box.state = config.shortcut.modifiers.contains(entry.modifier) ? .on : .off
            modifiers.append((box, entry.modifier)); view.addSubview(box)
        }
        ...
        keys.addItems(withTitles: ShortcutKey.emergency.map(\.name))
        keys.selectItem(at: ShortcutKey.emergency.firstIndex { $0.code == config.shortcut.key } ?? 0)
    ...
    func editShortcut(_ value: Shortcut) {
        enabled.state = value.enabled ? .on : .off
        keys.selectItem(at: ShortcutKey.emergency.firstIndex { $0.code == value.key } ?? 0)
        for (box, modifier) in modifiers { box.state = value.modifiers.contains(modifier) ? .on : .off }
    ...
    private func apply(_ change: Change) {
        ...
        case let .shortcut(value):
            ...
            if value.enabled && value.modifiers.count < 2 {
